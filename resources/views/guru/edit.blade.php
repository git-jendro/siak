    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="user" action="{{ route('guru.update', [$item->id]) }}" method="post"
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <input type="text" class="form-control @error('nuptk-' . $item->id) is-invalid @enderror"
                                name="nuptk-{{ $item->id }}" value="{{ $item->nuptk }}"
                                placeholder="NUPTK">
                            @error('nuptk-' . $item->id)
                                <div id="edit-error-{{ $item->id }}" class="mt-2 error invalid-feedback d-block w-100">
                                    {{ $message }}
                        ...
                        <div class="col-sm-6">
                            <select class="form-control @error('status-' . $item->id) is-invalid @enderror"
                                ...
                                <option value="0" {{ $item->status == 1 ? 'selected' : '' }}>Aktif
                                </option>
                                <option value="1" {{ $item->status == 0 ? 'selected' : '' }}>Tidak Aktif
                                </option>
                            </select>
                        ...
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <select class="form-control @error('agama_id-' . $item->id) is-invalid @enderror"
                                ...
                                @foreach ($agama as $agm)
                                    <option value="{{ $agm->id }}"
                                        {{ $item->agama_id == $agm->id ? 'selected' : '' }}>{{ $agm->nama }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <input type="text"
                                class="form-control @error('pendidikan-' . $item->id) is-invalid @enderror"
                                name="pendidikan-{{ $item->id }}" value="{{ $item->pendidikan }}"
                                placeholder="Pendidikan Terkahir">
                            @error('pendidikan-' . $item->id)
                                <div id="edit-error-{{ $item->id }}" class="mt-2 error invalid-feedback d-block w-100">
                                    {{ $message }}
                                </div>
                            @enderror
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control @error('jurusan-' . $item->id) is-invalid @enderror"
                                name="jurusan-{{ $item->id }}" value="{{ $item->jurusan }}"
                                placeholder="Jurusan Pedidikan Terakhir">
                            @error('jurusan-' . $item->id)
                                <div id="edit-error-{{ $item->id }}" class="mt-2 error invalid-feedback d-block w-100">
                                    {{ $message }}
                                </div>
                            @enderror
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <input type="password"
                                ...
                                name="password-{{ $item->id }}" value="" placeholder="Password">

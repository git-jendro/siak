    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Form Edit Staff</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="user" action="{{ route('staff.update', [$item->id]) }}" method="post"
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <select class="form-control @error('jabatan_id-' . $item->id) is-invalid @enderror"
                                name="jabatan_id-{{ $item->id }}">
                                <option value="">Pilih Jabatan</option>
                                @foreach ($jabatan as $jab)
                                    <option value="{{ $jab->id }}"
                                        {{ $item->jabatan_id == $jab->id ? 'selected' : '' }}>{{ $jab->nama }}
                                    </option>
                                @endforeach
                            </select>
                            @error('jabatan_id-' . $item->id)
                                <div id="edit-error-{{ $item->id }}" class="mt-2 error invalid-feedback d-block w-100">
                                    {{ $message }}
                                </div>
                            @enderror
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <input type="text" class="form-control @error('username-' . $item->id) is-invalid @enderror"
                                name="username-{{ $item->id }}" value="{{ $item->user->username }}"

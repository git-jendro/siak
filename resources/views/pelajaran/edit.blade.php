                ...
                <form class="user" action="{{ route('pelajaran.update', [$item->id]) }}" method="post"
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            ...
                            <input type="text"
                            class="form-control form-control-user @error('nama-'.$item->id) is-invalid @enderror" name="nama-{{ $item->id }}"
                            placeholder="Nama Mata Pelajaran" value="{{ $item->nama }}">
                            @error('nama-'.$item->id)
                        ...
                        <div class="col-sm-6">
                            <input type="number"
                                class="form-control form-control-user @error('kkm-'.$item->id) is-invalid @enderror" name="kkm-{{ $item->id }}"
                                placeholder="KKM" value="{{ $item->kkm }}">
                            @error('kkm-'.$item->id)
                                <div id="edit-error-{{ $item->id }}"
                                    class="px-3 mt-2 error invalid-feedback d-block w-100">
                                    {{ $message }}
